...
- [ ] **Phase 6 — Welt-Flavor** (**73 Einheiten**, extrahiert und kuratiert
      ...
      Spiel trägt an Welt-Flavor 5.628 Zeichen (Median 83 pro Einheit,
      min 25, max 139). Zwei Extraktionswege in `extract-manifest.js`:
      Konstanten-Listen über `WORLD_SOURCES` (48 Einheiten, `id` als
      Schlüssel) und `script/content.js` per Textmuster (25 Ortskarten,
      ID als Kurz-Hash wie bei den Objectives — dort steckt der Text in
      Render-Funktionen und `id:` ist teils eine Variable). Ausgelassen:
      `WILD_PET_DEFS` (nur `hint`-Labels und `bonusFn`-Templates),
      ...
      *Stand 06.09.2026 (Nachtlauf): 47/73 — Monster (16), Markt (13),
      Expedition (6) und Haustiere (3) sind komplett, Alchemie steht bei 9/10.
      Offen: Orte 25, Alchemie 1.*
...
Phase 4 bis ~05.08., Phase 5 bis ~24.08. **Korrektur 26.08.2026 nach der
Phase-6-Extraktion:** Phase 5 läuft bis ~30./31.08., Phase 6 umfasst statt
der geschätzten ~250 nur 73 Einheiten und ist damit bei 10/Tag um den
07./08.09.2026 herum durch — nicht Ende September. Phase 7 ist gestrichen,
der Plan endet mit Phase 6.
...
- 26.08.2026 22:22 — Batch: 9 Dateien (quest-archivRecherche-dok3 … quest-dieKonfrontation-unstarted), 46 s Audio, komplett. Gesamt: 288/331 im Manifest.
- 26.08.2026 22:23 — Tagesbilanz: 10 Requests, 9 erfolgreich, **1× HTTP 503** (`quest-seleWissen-active`, „high demand", Position 4 im Batch) — damit reißt die Serie von neun fehlerfreien Zehnerläufen in Folge. Das 503 kostet einen Tages-Slot (belegt seit 28.07.), der Knoten steht in `progress.failed` und läuft im nächsten Batch automatisch als erste Einheit wieder mit; ein Same-Day-Retry entfällt, weil die Tagessumme mit dem Durchlauf bereits bei 10 liegt. Kein 400, keine Leerantwort, keine Sonden. Vierter Lauf komplett über `tts/nightly.js --push`, eine Freigabe, kein Zwischenfall; `publish-audio.js` veröffentlichte alle 9 neuen Paare nach `tts/audio/`, 0 zurückgehalten, Opus-Konvertierung und Alignment fehlerfrei; Commit `962947a` gepusht. Der Gegencheck vor dem Lauf (279 Einträge in `progress.json`, 279 WAVs in `tts/output/`, neuester vom 24.08. 23:06, 558 Dateien = 279 vollständige Paare in `tts/audio/`, 0 fehlgeschlagene) deckt sich mit der letzten Log-Zeile — kein verschwundener Tagesertrag. **Am 25.08. lief kein Batch** (keine Log-Zeile, keine Dateien mit diesem Zeitstempel); sein Kontingent ist ungenutzt verfallen, weil auch kein Lauf vor 09:00 des 26.08. stattfand — genau der Nachhol-Fall aus der Fenster-Regel vom 20.08. Phase 5 steht bei 134/177 (Quests 134/139, Objectives 0/38); die zum Zeitpunkt des Laufs 43 offenen Einheiten des Manifests waren **ausnahmslos** Phase 5, davon 38 am Stück die Objectives — bei 10/Tag ist die Phase um den 30./31.08. herum durch. **Phase 6 (Welt-Flavor) ist mit diesem Lauf extrahiert**, siehe nächste Zeile — der von der Chronik des 24.08. auf spätestens den 29.08. datierte Arbeitsschritt ist damit vorgezogen und das Manifest läuft nach Phase 5 nicht leer.
- 26.08.2026 22:41 — **Phase 6 (Welt-Flavor) extrahiert — drei Tage vor dem selbst gesetzten Termin, und deutlich kleiner als geschätzt.** Statt der seit Wochen im Plan stehenden „~250 Einheiten“ trägt das Spiel an Welt-Flavor genau **73**: Monster 16, Ortskarten 25, Markt 13, Alchemie 10, Expedition 6, Haustiere 3 — zusammen 5.628 Zeichen, Median 83 pro Einheit. Das Manifest wächst damit von 331 auf **404** Einheiten. Die alte Schätzung stammte aus einer Zählung von `desc:`-Vorkommen quer durch `script/`, ohne Abzug der reinen Zahlen- und Hinweislabels — wieder ein Fall für die Regel, vor der Umsetzung zu messen statt zu schätzen. **Zwei Extraktionswege**, weil die Quellen unterschiedlich gebaut sind: Zehn Konstanten-Listen (`WORLD_SOURCES` in `extract-manifest.js`) liefern 48 Einheiten mit stabilem `id`-Schlüssel; die 25 Ortskarten stecken dagegen **in den Render-Funktionen** von `script/content.js` (`const places = [...(gameFlags.jobUnlocked ? [{…}] : [])]`) und lassen sich nicht als Konstante auslesen. Sie kommen deshalb per Textmuster heraus, mit Kurz-Hash als ID — derselbe Weg wie bei den Objectives, und aus demselben Grund: `id:` ist dort teils eine Variable (`CONTENT.VELMARK_MARKT`), also kein Literal. **Stolperstelle dabei:** Die erste Fassung des Musters verlangte `name:` vor `desc:` und fand nur 19 der 25 Karten — die fünf Felder des Valdris-Dossiers benutzen `label:` und schreiben `desc:` in dieselbe Zeile. Ohne den Gegencheck „wie viele `desc:`-Literale stehen überhaupt in der Datei“ wären sie stumm unter den Tisch gefallen. **Bewusst ausgelassen** (steht als Kommentar im Skript): `WILD_PET_DEFS` trägt kein `desc`, nur `hint`-Spielhinweise und `bonusFn`-Templates; `ALCHEMIE_MILESTONES` sind reine Zahlenanzeigen wie schon `getExplicitGoalText()` in Phase 5; `VENDORS` hat keinen Beschreibungstext. Alle 73 sind Ich-Perspektive oder Erzählerton → Iapetus, kein NPC-Stil, damit auch kein Kandidat für die 400er-Wechselwirkung. **Gegengeprüft vor dem Commit:** 404 Einheiten mit 404 eindeutigen IDs, keine der 288 bereits vertonten IDs verlorengegangen (sonst hätte der Batch sie ein zweites Mal generiert), kein `${…}`-Rest im Text, kein Eintrag ohne Titel. Kosten: null Requests — die Extraktion liest nur Dateien. Praktische Folge: Wenn Phase 5 um den 30./31.08. leerläuft, steht der Nachschub bereit; Phase 6 ist bei 10/Tag um den 07./08.09.2026 durch, und damit der ganze Plan.
- 26.08.2026 22:42 — **Zwei Nachbesserungen an `tts/nightly.js`, beide durch die Phase-6-Extraktion aufgedeckt.** (1) Die Standzeile zählte die laufende Phase hart über die ID-Präfixe `quest-`/`objective-` und hätte ab Ende August stur „Phase 5: 177/177“ gemeldet, während längst Phase 6 läuft. Sie nimmt jetzt die niedrigste Phase mit offenen Einheiten aus `u.phase`, das ohnehin in jeder Manifest-Einheit steht — gleiche Ausgabe heute, richtig ab dem 31.08., und ohne Sonderfall. (2) `commitAndPush()` stagte nur `PLAN.md`, `progress.json`, `manifest.json` und `tts/audio/`: Der heutige Patch an `extract-manifest.js` wäre **stumm ungestaged liegengeblieben**, das gepushte Manifest also ohne das Skript, das es erzeugt. Jetzt steht dort `tts` als Ganzes — `tts/output/` und die `align/`-Arbeitsdateien hält `.gitignore` ohnehin draußen, und alles außerhalb von `tts/` bleibt unangetastet.
- 27.08.2026 23:11 — Batch: 10 Dateien (quest-seleWissen-active … objective-dc6effaf), 48 s Audio, komplett. Gesamt: 298/404 im Manifest.
- 27.08.2026 23:11 — `publish-audio.js`: 10 neue Paare nach `tts/audio/` veröffentlicht (Opus + words.json), 0 zurückgehalten. Opus-Konvertierung und Alignment liefen für alle 10 Einheiten fehlerfrei.
- 27.08.2026 23:12 — Tagesbilanz: 10 Requests, 10 erfolgreich, kein 400, kein 503, keine Leerantwort. Die am 26.08. gerissene Serie beginnt neu: erster fehlerfreier Zehnerlauf seit dem 503. **Der Tagesertrag ist ein Phasen-Teilabschluss:** `quest-seleWissen-active`, das gestern mit HTTP 503 stehen blieb und in `progress.failed` stand, lief als erste Einheit auf Anhieb mit HTTP 200 durch — damit ist das 503 bestätigt als das, wofür es seit dem 28.07. gilt: ein vorübergehender Kapazitätsengpass ohne Einheitenbezug, der genau einen Tages-Slot kostet und sich am Folgetag von selbst erledigt. `progress.failed` ist wieder leer. Mit den vier Knoten von `dieKonfrontation` sind die **139 Quest-Beschreibungen vollständig**; ab Position 6 des Batches laufen die Objectives, die letzte Teilmenge von Phase 5. Fünfter Lauf komplett über `tts/nightly.js --push`, eine Freigabe, kein Zwischenfall; Commit `051a5e0` gepusht. Der Gegencheck vor dem Lauf (288 Einträge in `progress.json`, 288 WAVs in `tts/output/`, neuester vom 26.08. 22:22, 576 Dateien = 288 vollständige Paare in `tts/audio/`, 1 fehlgeschlagene Einheit) deckt sich mit der letzten Log-Zeile — kein verschwundener Tagesertrag. **Die Sitzung pausierte wieder rund eine Stunde:** Die geplante Aufgabe startete um 22:09, der Batch lief erst 23:07–23:11 — dasselbe Muster wie in den Nächten 17./18.08. und 21./22.08., diesmal ohne Datumswechsel und damit ohne jede Folge für die Quota (Fenster-Umschlag ~09:00). Phase 5 steht bei 144/177 (Quests 139/139, Objectives 5/38); die 106 offenen Einheiten des Manifests sind 33 Objectives und 73 Welt-Flavor-Texte aus Phase 6. Bei 10/Tag ist Phase 5 am 30.08. durch, Phase 6 um den 07.09. — der Plan endet dann.
- 28.08.2026 22:20 — Batch: 10 Dateien (objective-3d180881 … objective-6a095a52), 105 s Audio, komplett. Gesamt: 308/404 im Manifest.
...
- 02.09.2026 22:16 — Batch: 10 Dateien (world-monster-schattenwolf … world-markt-axt), 85 s Audio, komplett. Gesamt: 348/404 im Manifest.
- 02.09.2026 22:17 — Tagesbilanz: 10 Requests, 10 erfolgreich, kein 400, kein 503, keine Leerantwort — **sechster fehlerfreier Zehnerlauf in Folge** seit dem 503 vom 26.08. `progress.failed` bleibt leer. Zehnter Lauf komplett über `tts/nightly.js --push`, eine Freigabe, kein Zwischenfall; `publish-audio.js` veröffentlichte alle 10 neuen Paare nach `tts/audio/`, 0 zurückgehalten, Opus-Konvertierung und Alignment fehlerfrei; Commit `ca69b3f` gepusht. Der Gegencheck vor dem Lauf (338 Einträge in `progress.json`, 338 WAVs in `tts/output/`, jüngster vom 01.09. 23:29, 0 fehlgeschlagene) deckt sich mit der Batch-Zeile des Vortags — kein verschwundener Tagesertrag. Erster Lauf seit dem 30.08. wieder ohne Sitzungspause: geplante Aufgabe 22:09, Batch 22:10–22:16 (an den Dateizeitstempeln in `tts/output/` abgelesen, nicht geschätzt — siehe die Korrektur vom 30.08.). **Der Tagesertrag ist eine komplette Gruppe:** Mit den neun Monstern dieses Batches sind die **16 Monster-Beschreibungen von Phase 6 fertig**, Position 10 lief bereits in die Markt-Texte. Damit liegt der zweite Gruppenwechsel innerhalb eines Batches in Folge — die Standzeile meldet unbeirrt „Phase 6: 17/73", weil sie seit dem 26.08. nach `u.phase` zählt und nicht nach ID-Präfix; Gruppen innerhalb einer Phase unterscheidet sie bewusst nicht. **Auffällig ist der Längensprung nach unten:** Die Monster tragen 68–129 Zeichen und 6,4–11,9 s, der erste Markt-Text (`world-markt-axt`) nur 27 Zeichen und 3,2 s. Die Markt-Einträge sind Gegenstandsbeschreibungen und damit die kürzeste Teilmenge von Phase 6 — die 85 s dieses Laufs sind die letzte lange Bilanz für eine Weile, der Ø sinkt in den nächsten beiden Läufen absehbar Richtung Quest-Kurztext-Niveau (Ø 5 s). Phase 6 steht bei 17/73; die 56 offenen Einheiten des Manifests sind ausnahmslos Welt-Flavor (25 Orte, 12 Markt, 10 Alchemie, 6 Expedition, 3 Pets). Bei 10/Tag ist Phase 6 um den 08.09. durch — und damit der ganze Plan, denn Phase 7 ist seit dem 12.08. gestrichen.
- 03.09.2026 22:16 — Batch: 10 Dateien (world-markt-spitzhacke … world-markt-velmark_hafenfisch), 60 s Audio, komplett. Gesamt: 358/404 im Manifest.
- 03.09.2026 22:17 — Tagesbilanz: 10 Requests, 10 erfolgreich, kein 400, kein 503, keine Leerantwort — **siebter fehlerfreier Zehnerlauf in Folge** seit dem 503 vom 26.08. `progress.failed` bleibt leer. Elfter Lauf komplett über `tts/nightly.js --push`, eine Freigabe, kein Zwischenfall; `publish-audio.js` veröffentlichte alle 10 neuen Paare nach `tts/audio/`, 0 zurückgehalten, Opus-Konvertierung und Alignment fehlerfrei; Commit `b5309fa` gepusht. Der Gegencheck vor dem Lauf (348 Einträge in `progress.json`, 348 WAVs in `tts/output/`, jüngster vom 02.09. 22:16, 0 fehlgeschlagene) deckt sich mit der Batch-Zeile des Vortags — kein verschwundener Tagesertrag. Zweiter Lauf in Folge ohne Sitzungspause: geplante Aufgabe 22:09, Batch 22:10:29–22:16:06 (an den Dateizeitstempeln abgelesen, nicht geschätzt). **Der Tagesertrag ist reine Marktware:** zehn Gegenstands- und Speisenbeschreibungen (`world-markt-spitzhacke` … `world-markt-velmark_hafenfisch`), damit sind 11 der 13 Markt-Texte vertont; die letzten zwei laufen morgen als Auftakt in die Ortskarten. **Die gestrige Prognose zur Länge trifft zu, aber nur zur Hälfte:** Der Ø fällt von 7,7 s auf **6,0 s** (60 s auf 10 Einheiten) — der Absturz auf Quest-Kurztext-Niveau (Ø 5 s) blieb aus, weil die Markt-Gruppe zweigeteilt ist: Werkzeuge und Grundnahrungsmittel liegen bei 32–54 Zeichen und 2,7–4,6 s, die Regionalspezialitäten (`kaffee`, `lethkar_*`, `velmark_hafenfisch`) dagegen bei 63–100 Zeichen und 5,3–13,5 s. `world-markt-kaffee` ist mit 13,5 s auf 100 Zeichen der längste Einzeltext von Phase 6 bisher und fällt aus dem Verhältnis der übrigen (rund 7,4 Zeichen/s statt der sonst üblichen 11–12) — die Stimme nimmt sich dort erkennbar mehr Zeit; kein Fehlerbild, nur eine Auffälligkeit fürs Protokoll. Phase 6 steht bei 27/73; die 46 offenen Einheiten des Manifests sind ausnahmslos Welt-Flavor (25 Orte, 10 Alchemie, 6 Expedition, 3 Pets, 2 Markt). Die 25 Ortskarten sind mit Abstand die größte Restgruppe und die einzige, die per Textmuster statt aus Konstanten extrahiert wurde — der erste Batch, der in sie hineinläuft, ist damit der Prüfstein für den zweiten Extraktionsweg vom 26.08. Bei 10/Tag ist Phase 6 um den 08.09. durch — und damit der ganze Plan, denn Phase 7 ist seit dem 12.08. gestrichen.
- 05.09.2026 22:41 — Batch: 10 Dateien (world-markt-velmark_brot … world-pet-wolfWelpe), 77 s Audio, komplett. Gesamt: 368/404 im Manifest.
- 05.09.2026 22:42 — Tagesbilanz: 10 Requests, 10 erfolgreich, kein 400, kein 503, keine Leerantwort — **achter fehlerfreier Zehnerlauf in Folge** seit dem 503 vom 26.08. `progress.failed` bleibt leer. Zwölfter Lauf komplett über `tts/nightly.js --push`, eine Freigabe, kein Zwischenfall; `publish-audio.js` veröffentlichte alle 10 neuen Paare nach `tts/audio/`, 0 zurückgehalten, Opus-Konvertierung und Alignment fehlerfrei; Commit `9402640` gepusht. Der Gegencheck vor dem Lauf (358 Einträge in `progress.json`, 358 WAVs in `tts/output/`, 716 Dateien = 358 vollständige Paare in `tts/audio/`, jüngster Eintrag vom 03.09. 22:16, 0 fehlgeschlagene) deckt sich mit der Batch-Zeile des Vortags — kein verschwundener Tagesertrag. **Am 04.09. lief kein Batch** (keine Log-Zeile, keine Dateien mit diesem Zeitstempel in `tts/output/`); da auch kein Lauf vor 09:00 des 05.09. stattfand, ist sein Kontingent nach der Fenster-Regel vom 20.08. ungenutzt verfallen — derselbe Fall wie am 25.08. und am 31.08. Die Sitzung pausierte diesmal rund 25 Minuten: geplante Aufgabe 22:10, Batch 22:35:28–22:41:10 (an den Dateizeitstempeln abgelesen, nicht geschätzt — siehe die Korrektur vom 30.08.). **Der Tagesertrag sind zwei komplette Gruppen:** Mit `world-markt-velmark_brot` und `world-markt-velmark_kaviar` sind die **13 Markt-Texte fertig**, mit den sechs Einträgen ab Position 3 auch die **komplette Expedition-Gruppe**; Position 9 und 10 liefen bereits in die Haustiere, von denen nur noch `world-pet-stadtfalke` aussteht. Damit liegen in diesem Batch **zwei Gruppenwechsel**, der dritte Lauf in Folge mit mindestens einem — die Standzeile meldet unbeirrt „Phase 6: 37/73", weil sie seit dem 26.08. nach `u.phase` zählt. **Zur Länge:** Der Ø steigt von 6,0 s auf 7,7 s zurück, weil die Expedition-Texte mit 91–139 Zeichen die längste Teilmenge von Phase 6 sind (`ruins_of_aethren`: 139 Zeichen, 12,0 s — der längste Einzeltext der Phase bisher). Das Verhältnis liegt dort bei stabilen 11,6–12,2 Zeichen/s, also im Normalbereich; der Ausreißer `world-markt-kaffee` vom Vortag (7,4 Zeichen/s) bleibt ein Einzelfall. Phase 6 steht bei 37/73; die 36 offenen Einheiten des Manifests sind ausnahmslos Welt-Flavor (25 Orte, 10 Alchemie, 1 Pet). **Die 25 Ortskarten sind damit ab dem übernächsten Batch die einzige Restgruppe** — und die einzige, die per Textmuster statt aus Konstanten extrahiert wurde; der Prüfstein für den zweiten Extraktionsweg vom 26.08. steht weiterhin aus, weil die Manifest-Reihenfolge Alchemie vorzieht. Bei 10/Tag ist Phase 6 um den 09.09. durch — und damit der ganze Plan, denn Phase 7 ist seit dem 12.08. gestrichen. Nebenbei bereinigt: Die Batch-Zeile vom 02.09. 22:16 stand doppelt in der Chronik, das Duplikat ist raus.
- 06.09.2026 22:29 — Batch: 10 Dateien (world-pet-stadtfalke … world-alchemie-aspektmeister), 53 s Audio, komplett. Gesamt: 378/404 im Manifest.
- 06.09.2026 22:31 — Tagesbilanz: 10 Requests, 10 erfolgreich, kein 400, kein 503, keine Leerantwort — **neunter fehlerfreier Zehnerlauf in Folge** seit dem 503 vom 26.08. `progress.failed` bleibt leer. Dreizehnter Lauf komplett über `tts/nightly.js --push`, eine Freigabe, kein Zwischenfall; `publish-audio.js` veröffentlichte alle 10 neuen Paare nach `tts/audio/`, 0 zurückgehalten, Opus-Konvertierung und Alignment fehlerfrei; Commit `62a2def` gepusht. Der Gegencheck vor dem Lauf (368 Einträge in `progress.json`, 368 WAVs in `tts/output/`, 736 Dateien = 368 vollständige Paare in `tts/audio/`, jüngster Eintrag vom 05.09. 22:41, 0 fehlgeschlagene) deckt sich mit der Batch-Zeile des Vortags — kein verschwundener Tagesertrag. Anders als am 25.08., 31.08. und 04.09. ist kein Tag ausgefallen: der Vorlauf war am 05.09., dieser hier am 06.09., beide innerhalb ihres Quota-Tages (Fenster-Umschlag ~09:00). Die Sitzung pausierte 14 Minuten: geplante Aufgabe 22:09, Batch 22:23:30–22:28:58 (an den Dateizeitstempeln in `tts/output/` abgelesen, nicht geschätzt — siehe die Korrektur vom 30.08.). **Der Tagesertrag schließt die Haustiere ab:** `world-pet-stadtfalke` war der letzte der drei Pets, ab Position 2 lief der Batch durch die Alchemie, die nach neun Einheiten bei 9/10 steht. Damit ist der vierte Lauf in Folge mit mindestens einem Gruppenwechsel; die Standzeile meldet unbeirrt „Phase 6: 47/73", weil sie seit dem 26.08. nach `u.phase` zählt und Gruppen bewusst nicht unterscheidet. **Zur Länge — ein Gruppenmuster, kein Ausreißer:** Der Ø fällt von 7,7 s auf 5,3 s (53 s auf 10 Einheiten), und bei den vier Element-Beschreibungen (`feuer`, `wasser`, `erde`, `luft`, 26–38 Zeichen) liegt das Verhältnis bei nur 5,4–7,6 Zeichen/s statt der sonst üblichen 11–12. Das ist keine Wiederholung des `world-markt-kaffee`-Falls vom 03.09. (100 Zeichen bei 7,4 Zeichen/s), sondern der erwartbare Effekt kurzer Texte: Anlauf und Ausklang der Aufnahme sind eine feste Größe und schlagen bei 26 Zeichen viel stärker durch als bei 139. Die längeren Alchemie-Texte desselben Batches (`aether`, `aspektmeister`, je 63 Zeichen) liegen mit 10,0 und 11,7 Zeichen/s wieder im Normalbereich — die Auffälligkeit hängt also an der Textlänge, nicht an der Gruppe. Phase 6 steht bei 47/73; die 26 offenen Einheiten des Manifests sind 25 Ortskarten und eine letzte Alchemie-Einheit (`world-alchemie-alchemistischesGedaechtnis`). **Ab morgen sind die Ortskarten die einzige Restgruppe** — und damit steht endlich der seit dem 03.09. angekündigte Prüfstein an: Sie sind die einzigen Einheiten, die per Textmuster aus `script/content.js` extrahiert wurden statt aus Konstanten (26.08.), und der erste Batch, der in sie hineinläuft, zeigt, ob dieser zweite Weg saubere Texte geliefert hat. Bei 10/Tag ist Phase 6 um den 09.09. durch — und damit der ganze Plan, denn Phase 7 ist seit dem 12.08. gestrichen.
- 07.09.2026 22:33 — Batch: 10 Dateien (world-alchemie-alchemistischesGedaechtnis … world-ort-5131dd2c), 71 s Audio, komplett. Gesamt: 388/404 im Manifest.
